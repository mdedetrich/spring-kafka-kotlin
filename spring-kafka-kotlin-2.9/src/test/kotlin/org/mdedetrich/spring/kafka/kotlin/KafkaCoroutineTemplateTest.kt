...
class KafkaCoroutineTemplateTestConfig {
    @Bean
    fun producerFactory(): ProducerFactory<String, String> = MockProducerFactory(MockProducer(true, StringSerializer(), StringSerializer()))
...
class KafkaCoroutineTemplateTest {
    ...
    // skips them, no failure reported. JUnit 6.1.2 (3.0+) does support them natively. Confirmed via a
    // real test run showing these methods missing entirely from the result count, not merely failing.
    @Test
    @Timeout(10)
    fun `KafkaCoroutineTemplate works with a real DefaultKafkaProducerFactory, propagating send failures`() =
        runBlocking {
    ...
    @Test
    fun `send suspends and actually delegates through KafkaTemplate to the underlying Producer`() =
        runBlocking {
            val mockProducer = MockProducer(true, StringSerializer(), StringSerializer())
            val kafkaTemplate = KafkaTemplate(MockProducerFactory(mockProducer))
            val coroutineTemplate = KafkaCoroutineTemplate(kafkaTemplate)
