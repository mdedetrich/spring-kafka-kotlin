 ...
 * - Operations returning a [org.springframework.util.concurrent.ListenableFuture] are `suspend` functions
 *   instead.
 ...
 */
public class KafkaCoroutineTemplate<K, V>(
    ...
    override suspend fun sendDefault(data: V?): SendResult<K?, V?> = delegate.sendDefault(data).completable().await()

    override suspend fun sendDefault(
        key: K & Any,
        data: V?,
    ): SendResult<K?, V?> = delegate.sendDefault(key, data).completable().await()

    override suspend fun sendDefault(
        partition: Int,
        key: K & Any,
        data: V?,
    ): SendResult<K?, V?> = delegate.sendDefault(partition, key, data).completable().await()
    ...
    override suspend fun sendDefault(
        ...
        key: K & Any,
        data: V?,
    ): SendResult<K?, V?> = delegate.sendDefault(partition, timestamp, key, data).completable().await()

    override suspend fun send(
        topic: String,
        data: V?,
    ): SendResult<K?, V?> = delegate.send(topic, data).completable().await()

    override suspend fun send(
        topic: String,
        key: K & Any,
        data: V?,
    ): SendResult<K?, V?> = delegate.send(topic, key, data).completable().await()
    ...
    override suspend fun send(
        ...
        key: K & Any,
        data: V?,
    ): SendResult<K?, V?> = delegate.send(topic, partition, key, data).completable().await()
    ...
    override suspend fun send(
        ...
        key: K & Any,
        data: V?,
    ): SendResult<K?, V?> = delegate.send(topic, partition, timestamp, key, data).completable().await()

    override suspend fun send(record: ProducerRecord<K?, V?>): SendResult<K?, V?> = delegate.send(record).completable().await()

    override suspend fun send(message: Message<*>): SendResult<K?, V?> = delegate.send(message).completable().await()
    ...
    override fun <T> execute(callback: KafkaOperations.ProducerCallback<K?, V?, T>): T = delegate.execute(callback)

    override fun <T> executeInTransaction(callback: KafkaOperations.OperationsCallback<K?, V?, T>): T =
    ...
    @Deprecated("Deprecated in the 3.0.0 KafkaProducer.")
    override fun sendOffsetsToTransaction(offsets: Map<TopicPartition, OffsetAndMetadata>) {
        @Suppress("DEPRECATION")
        return delegate.sendOffsetsToTransaction(offsets)
    }
    ...
    override fun sendOffsetsToTransaction(
        ...
        @Suppress("DEPRECATION")
        return delegate.sendOffsetsToTransaction(offsets, consumerGroupId)
    }
    ...
    // receive(topic, partition, offset, pollTimeout) is genuinely nullable: KafkaTemplate.receive(...)
    // is @Nullable and its real implementation (receiveOne) returns null whenever the poll doesn.t yield
    // exactly one record within pollTimeout -- an ordinary, expected outcome (nothing arrived in time),
    // not an edge case.
