...
/**
 * The basic Kafka operations contract -- returning [org.springframework.util.concurrent.ListenableFuture]s
 * on this version ([java.util.concurrent.CompletableFuture] from spring-kafka 3.0).
 *
...
public interface KafkaCoroutineOperations<K, V> {
    /**
    ...
    public fun <T> execute(callback: KafkaOperations.ProducerCallback<K?, V?, T>): T
    ...
    public fun <T> executeInTransaction(callback: KafkaOperations.OperationsCallback<K?, V?, T>): T
     ...
     * group id is obtained from [org.springframework.kafka.support.KafkaUtils.getConsumerGroupId].
     * It is not necessary to call this method if the operations are invoked on a listener
    ...
    public fun sendOffsetsToTransaction(offsets: Map<TopicPartition, OffsetAndMetadata>): Unit = throw UnsupportedOperationException()
     ...
     * which just `.await()` an already-async [org.springframework.util.concurrent.ListenableFuture]. See
