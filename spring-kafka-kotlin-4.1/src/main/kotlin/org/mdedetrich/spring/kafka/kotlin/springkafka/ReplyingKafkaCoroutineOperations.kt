 ...
 * [org.springframework.kafka.requestreply.RequestReplyFuture]/[java.util.concurrent.CompletableFuture].
 *
 * @param <K> the request key type.
 * @param <V> the request value type.
 * @param <R> the reply value type.
 */
public interface ReplyingKafkaCoroutineOperations<K : Any, V : Any, R : Any> {
    /**
     ...
     */
    public suspend fun <P : Any> sendAndReceive(
        message: Message<*>,
     ...
     * @return the send result and reply message, as independent deferreds.
     */
    public fun <P : Any> sendAndReceiveDeferred(
        message: Message<*>,
        typeReference: ParameterizedTypeReference<P>,
    ...
    /**
     * Set to true if the correlation id is binary rather than a string representation.
     * @param binaryCorrelation true for binary correlation.
     */
    public fun setBinaryCorrelation(binaryCorrelation: Boolean)
}
