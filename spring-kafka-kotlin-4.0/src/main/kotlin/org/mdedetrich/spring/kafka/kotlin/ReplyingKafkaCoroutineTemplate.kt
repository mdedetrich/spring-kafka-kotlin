 ...
 */
public class ReplyingKafkaCoroutineTemplate<K : Any, V : Any, R : Any>(
    ...
    override suspend fun <P : Any> sendAndReceive(
        message: Message<*>,
    ...
    // getSendFuture() is @Nullable on this version (was non-null through 3.3) -- but it's always
    // populated by the time sendAndReceive(...) returns the future to us (set internally the moment the
    // request is handed to the producer, before this method call can even observe the future), so !! is
    // safe here and at every other getSendFuture() call site below.
    ...
    override fun <P : Any> sendAndReceiveDeferred(
        message: Message<*>,
        typeReference: ParameterizedTypeReference<P>,
    ...
    // assignedReplyTopicPartitions is @Nullable on this version (was non-null through 3.3) -- null and
    // empty are equivalent here (no partitions currently assigned), so this stays non-null for API
    // parity with the 2.9-3.3 wrappers.
    override fun getAssignedReplyTopicPartitions(): Collection<TopicPartition> = delegate.assignedReplyTopicPartitions ?: emptyList()
     ...
     * provided.
     *
     * See [KafkaCoroutineTemplate.defaultTopic] for why this stays non-null despite the delegate's
     * getter being `@Nullable` on this version.
     */
