...
class ReplyingKafkaCoroutineTemplateTest {
    ...
    @Test
    @Timeout(10)
    suspend fun `typed sendAndReceive propagates failure when the reply container was never started`() {
        val replyingKafkaTemplate = unstartedReplyingKafkaTemplate()
        val coroutineTemplate = ReplyingKafkaCoroutineTemplate(replyingKafkaTemplate)
        val message =
            MessageBuilder
    ...
    @Test
    @Timeout(10)
    suspend fun `typed sendAndReceive with an explicit reply timeout also returns a properly typed Message`() =
        coroutineScope {
            val replyingKafkaTemplate = mockProducerReplyingKafkaTemplate()
    ...
    @Test
    @Timeout(10)
    suspend fun `reified sendAndReceive avoids the manual ParameterizedTypeReference boilerplate`() =
        coroutineScope {
            val replyingKafkaTemplate = mockProducerReplyingKafkaTemplate()
    ...
    @Test
    @Timeout(10)
    suspend fun `sendAndReceiveDeferred surfaces a send failure on sendResult independently of reply`() {
