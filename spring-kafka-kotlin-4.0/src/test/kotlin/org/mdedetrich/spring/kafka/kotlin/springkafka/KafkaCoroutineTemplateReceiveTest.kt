...
class KafkaCoroutineTemplateReceiveTest {
    @Test
    @Timeout(10)
    suspend fun `receive with the default dispatcher runs off the calling coroutine's thread`() {
        ...
        // receive(...) is genuinely nullable on this version -- see KafkaCoroutineOperations' note --
        // but ThreadRecordingKafkaTemplate always fabricates a real record, never null.
    ...
    @Test
    @Timeout(10)
    suspend fun `receive with a configured dispatcher runs on that dispatcher`() {
    ...
    @Test
    @Timeout(10)
    suspend fun `receive with a null dispatcher runs directly on the calling coroutine's thread`() {
        val callingThreadName = currentThreadName()
        val delegate = ThreadRecordingKafkaTemplate(mockProducerFactory())
