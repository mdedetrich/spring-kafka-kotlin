 ...
 * infrastructure (e.g. [org.springframework.messaging.handler.invocation.InvocableHandlerMethod]) uses to represent "a method plus the instance to invoke
 * it on," rather than the two as separate, uncorrelated parameters.
 ...
 * ```
 *
 * To run the listener body on a specific dispatcher, [kotlinx.coroutines.withContext] works the same way:
 * `withContext(Dispatchers.IO) { processMessage() }` -- see [KafkaListenerCoroutineHookAspect] for how
 * this module's backport actually bridges into and out of `processMessage`, which differs from the
 * `hook-aspect-native` module's own approach despite an identical public API here.
 *
