...
One `gradle` entry at root (covers the version catalog) plus one for `build-logic` (its own separate
Gradle project, directory-scoped for Dependabot even though it now shares the root's version catalog
file — see above). Ignore rules classify every dependency as:
